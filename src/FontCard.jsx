        ...
        <>
            <div className="font-card">
                ...
                <dialog closedby="any" ref={deleteRef} disabled={loadDelete} className="confirm">
                    <p style={{ fontFamily: fontFamily }}>{text[settings.language].deleteFont}</p>
                    <button onClick={deleteFont} style={{ fontFamily: fontFamily }}>{
                        loadDelete ? <span className="loader" style={{ width: "1rem", height: "1rem" }} />
                        : text[settings.language].confirm[1]
                    }</button>
                    <button onClick={() => closeModal(deleteRef)} style={{ fontFamily: fontFamily }}>{text[settings.language].confirm[2]}</button>
                </dialog>
            </div>
        </>
    )
